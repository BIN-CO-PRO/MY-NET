    ...
    <div className="container py-16 md:py-20 animate-fade-in">
      <div className="max-w-2xl mb-10">
        ...
        <p className="text-muted-foreground mt-3 text-lg">
          Documents, datasets, designs, and resources I've shared publicly. Click to preview or download.
        </p>
      </div>
            ...
            <Card key={f.id} className="p-4 hover:border-primary/40 transition-colors group">
              ...
              <div className="mt-3 flex gap-2">
                ...
                <a
                  ...
                  className={`flex items-center justify-center gap-2 rounded-lg border border-border py-2 text-sm font-medium hover:bg-muted transition-colors ${isPreviewable(f.mime_type) ? "px-3" : "flex-1"}`}
                >
      ...
      <Dialog open={!!previewFile} onOpenChange={(open) => !open && setPreviewFile(null)}>
        <DialogContent className="max-w-3xl max-h-[85vh] overflow-hidden p-0">
          ...
          <DialogDescription className="sr-only">
            File preview — {previewFile ? formatBytes(previewFile.size_bytes) : ""}
          </DialogDescription>
          {previewFile && (
            <FilePreview file={previewFile} url={fileUrl(previewFile.storage_path)} />
          )}
        </DialogContent>
      </Dialog>
    </div>
    ...
    <div className="flex flex-col">
      <div className="flex items-center justify-between px-5 py-3 border-b border-border">
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center gap-2 rounded-lg border border-border px-3 py-1.5 text-sm font-medium hover:bg-muted transition-colors shrink-0"
        >
          <Download className="h-4 w-4" /> Download
        </a>
      </div>
      <div className="flex items-center justify-center bg-black/30 p-4 min-h-[200px] max-h-[65vh] overflow-auto">
        ...
        {mime.startsWith("video/") && (
          <video src={url} controls autoPlay className="max-w-full max-h-[60vh] rounded-lg" />
        )}
        ...
        {mime.includes("pdf") && (
          <iframe src={url} title={file.name} className="w-full h-[60vh] rounded-lg" />
        )}
        {!mime.startsWith("image/") && !mime.startsWith("video/") && !mime.startsWith("audio/") && !mime.includes("pdf") && (
